...
public class ProjectController {
    ...
    public UnifiedResult deleteById(@PathVariable("id") Integer id) throws Exception {
        ...
        try {
            ...
            LOGGER.error("{} delete project by id error. id = {}", LOGGER_PREFIX, id, e);
            return UnifiedResult.failure(e.getCode(), e.getMessage());
        }
    }
    ...
    @PostMapping("/page/my")
    @ApiOperation(value = "分页查询我发布的项目列表", notes = "分页查询我发布的项目列表")
    public UnifiedResult<PageData<ProjectModel>> pageByMyRelease(@RequestBody ProjectQueryRequest request) throws Exception {

        LOGGER.info("{} page my projects by request. request = {}", LOGGER_PREFIX, request, RequestUtil.getUserId().toString());
        return UnifiedResult.success(projectService.pageByMyRelease(request));
    }
}
